...
def evaluateGaussianKernel(X_test, t_test, X_train, t_train, h):
    ...
    # np.where(t)
    b = np.where(denom < np.spacing(1))
    # t_est[np.nonzero(denom < np.spacing(1))] = default_value
    t_est[b] = default_value
    if len(t_test) > 0:
...
X_n = X_n[:, 4].reshape(X_n.shape[0], 1)
...
for i in range(len(hs)):
    x_ev = np.arange(min(X_train), max(X_train), 1)
    # y_ev = evaluateGaussianKernel(x_ev, [], X_train, t_train, hs[i])
    [y_ev, test_err] = evaluateGaussianKernel(x_ev, t_test, X_train, t_train, hs[i])
